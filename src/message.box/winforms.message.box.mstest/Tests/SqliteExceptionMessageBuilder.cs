...
#pragma warning disable CS0618 // System.Data.SqlClient SQL Exception is obsolete
...
public class SqliteExceptionMessageBuilder() : IExceptionMessageBuilder
{
    ...
    public virtual string BuildMessage( StringBuilder stringBuilder, Exception exception, string handledExceptionSource )
    {
...
#endif
        int number = 0;
        if ( exception is SqliteException exp )
            number = exp.SqliteExtendedErrorCode;
        if ( exception.Source is not null && exception.Source.Length > 0 && (stringBuilder.Length == 0 || handledExceptionSource != exception.Source) )
        {
            ...
            string source = !(exception.GetType() == typeof( SqliteException )) ? exception.Source : ExceptionMessageBuilderResources.SqlServerSource;
            if ( number > 0 )
    ...
    public virtual string BuildTechnicalDetails( Exception exception )
    {
        ...
        if ( exception is SqliteException sqlException )
        {
            _ = stringBuilder1.Append( Environment.NewLine );
            _ = stringBuilder1.Append( "---------------" );
            _ = stringBuilder1.Append( Environment.NewLine );
            _ = stringBuilder1.Append( ExceptionMessageBuilderResources.SqliteInfo );
            _ = stringBuilder1.Append( Environment.NewLine );
            _ = stringBuilder1.Append( Environment.NewLine );
            _ = stringBuilder1.Append( string.Format( ExceptionMessageBuilderResources.Culture, ExceptionMessageBuilderResources.ErrorCode, sqlException.ErrorCode.ToString( CultureInfo.CurrentCulture ) ) );
            _ = stringBuilder1.Append( Environment.NewLine );
            _ = stringBuilder1.Append( string.Format( ExceptionMessageBuilderResources.Culture, ExceptionMessageBuilderResources.ExtendedErrorCode, sqlException.SqliteExtendedErrorCode.ToString( CultureInfo.CurrentCulture ) ) );
            _ = stringBuilder1.Append( Environment.NewLine );
        }
    ...
    public bool HasTechnicalDetails( Exception exception )
    {
        ...
        {
            if ( (exp.StackTrace is not null && exp.StackTrace.Length > 0) || exp.GetType() == typeof( SqliteException ) )
                return true;
    ...
    {
        return exception is SqliteException exp
            ? string.Format( ExceptionMessageBuilderResources.Culture, ExceptionMessageBuilderResources.ErrorSourceNumber, ExceptionMessageBuilderResources.SqlServerSource, exp.SqliteExtendedErrorCode )
    ...
    public string BuildAdvancedInfoProperties( Exception exception )
    {
        ...
        if ( exception is SqliteException sqlException )
        {
            _ = stringBuilder.Append( string.Format( ExceptionMessageBuilderResources.Culture, ExceptionMessageBuilderResources.SqlError, sqlException.SqliteExtendedErrorCode.ToString( CultureInfo.CurrentCulture ) ) );
