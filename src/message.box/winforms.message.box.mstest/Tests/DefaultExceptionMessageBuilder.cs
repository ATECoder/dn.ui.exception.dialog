...
/// <summary>   The default exception message builder. </summary>
/// <remarks>   2025-06-23. </remarks>
public class DefaultExceptionMessageBuilder() : IExceptionMessageBuilder
{
...
#endif
        if ( exception.Source != null && exception.Source.Length > 0 && (stringBuilder.Length == 0 || handledExceptionSource != exception.Source) )
        {
    ...
    public virtual string BuildTechnicalDetails( Exception exception )
    {
        ...
        if ( exception.StackTrace != null && exception.StackTrace.Length > 0 )
        {
            ...
            _ = builder.Append( "---------------" );
            _ = builder.Append( Environment.NewLine );
            _ = builder.Append( ExceptionMessageBuilderResources.CodeLocation );
            _ = builder.Append( Environment.NewLine );
            _ = builder.Append( Environment.NewLine );
            _ = builder.Append( exception.StackTrace );
            _ = builder.Append( Environment.NewLine );
        }
    ...
    public bool HasTechnicalDetails( Exception exception )
    {
        ...
        if ( exception is null ) throw new ArgumentNullException( nameof( exception ) );
#endif
        for ( Exception? exp = exception; exp != null; exp = exp.InnerException )
        {
            if ( exp.StackTrace != null && exp.StackTrace.Length > 0 )
                return true;
                ...
                try
                {
                    ...
                    foreach ( DictionaryEntry dictionaryEntry in ex.Data )
                    {
                        if ( dictionaryEntry.Key is not null && !(dictionaryEntry.Key.GetType() != typeof( string ))
                            && dictionaryEntry.Value is not null && !(dictionaryEntry.Value.GetType() != typeof( string )) )
                        {
                            ...
                            else if ( string.Compare( ( string ) dictionaryEntry.Key, 0, "HelpLink.", 0,
                                "HelpLink.".Length, true, CultureInfo.CurrentCulture ) == 0 )
                            {
                                if ( num++ > 0 )
                                    _ = stringBuilder.Append( '&' );
    ...
    {
        return string.Format( ExceptionMessageBuilderResources.Culture, ExceptionMessageBuilderResources.ErrorSource, exception.Source );
    }
